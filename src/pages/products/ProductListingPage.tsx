...
import { ProductOrderManager } from "../../components/products/ProductOrderManager";
...
export function ProductListingPage() {
	...
	const updateOrderMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast.success("Ordem de exibição atualizada com sucesso!");
		},
		onError: () => {
	...
	return (
		<div className="space-y-6">
			...
			{isOrderingMode && products && products.length > 0 ? (
				<ProductOrderManager
					products={products}
					onSave={(items) => updateOrderMutation.mutate(items)}
					onCancel={() => setIsOrderingMode(false)}
